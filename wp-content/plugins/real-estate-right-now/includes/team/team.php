...
function realestate_show_team()
{
    $RealestateAgents = realestate_reorder_terms();
    ...
    if ($qagents > 0) {
        ...
        foreach ($RealestateAgents as $term) {
          if($i % 3 == 0 or $i == 0)  
           ...
           // $image = aq_resize( $image, 800, 400, true ); //resize & crop the image
            if (empty($image)) {
                $image = REALESTATEIMAGES . 'imagenoavailable800x400_br.jpg';
            ...
            $output .= $name;
            $output .= '</div>'; //title
            $output .= '<div class="RealEstate_team_function">';
            $output .= $function;
            $output .= '</div>'; //function
        ...
        $output .= __("No Team Member Added!", "realestate") ;
    }
    $output .= '</div>'; //container
    return $output;
}
add_shortcode('realestate_team', 'realestate_show_team'); ?>